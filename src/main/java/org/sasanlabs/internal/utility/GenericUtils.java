...
public class GenericUtils {
    ...
    /**
     * This is the utility method for invoking the {@link ICustomVulnerableEndPoint} providing
     * {@code ParameterBean} which wraps around the HttpRequest bean.
     *
    ...
    public static <T> ResponseBean<T> invokeMethod(
        ...
        for (Method method : customVulnerableEndPoint.getClass().getMethods()) {
            if (method.isAnnotationPresent(VulnerabilityLevel.class)) {
                VulnerabilityLevel vulnerabilityLevel =
                        method.getAnnotation(VulnerabilityLevel.class);
                if (vulnerabilityLevel.value() == level) {
                    try {
                        return (ResponseBean<T>)
                                method.invoke(customVulnerableEndPoint, parameterBean);
                    } catch (IllegalAccessException e) {
                        throw new ServiceApplicationException(
                                "Access Issue, please check the visibility of the annotated method ",
                                e,
                                ExceptionStatusCodeEnum.INVALID_ACCESS,
                                method.getName());
                    } catch (IllegalArgumentException e) {
                        throw new ServiceApplicationException(
                                ExceptionStatusCodeEnum.INVALID_ARGUMENTS,
                                "Access Issue, please check the arguments of the annotated method ",
                                e,
                                ExceptionStatusCodeEnum.INVALID_ACCESS,
                                method.getName());
        ...
        throw new ServiceApplicationException(
                "Unable to find the Method for Level :- " + level,
                ExceptionStatusCodeEnum.UNAVAILABLE_LEVEL,
                level,
                customVulnerableEndPoint
                        .getClass()
