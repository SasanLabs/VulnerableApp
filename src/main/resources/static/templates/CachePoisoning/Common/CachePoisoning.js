const CONFIGS = {
  LEVEL_1: {
    ...
    banner: {
      show: true,
      label: "Attacker banner",
      placeholder: "FAKE URGENT UPDATE",
    },
  ...
  LEVEL_2: {
    ...
    banner: {
      show: true,
      label: "Filtered banner",
      placeholder: "<b>URGENT STATUS PAGE UPDATE</b>",
    },
    forwardedHost: { show: false },
    demoUser: { show: false },
    poisonBtn: "inline",
  ...
  LEVEL_3: {
    ...
    subtitle:
      "This level fixes the banner cache key, but still trusts <code>X-Forwarded-Host</code> when building absolute asset URLs. Use the same banner value for both requests and change only the attacker header.",
    banner: {
      show: true,
      label: "Shared banner key",
      placeholder: "shared-status-page",
    },
    forwardedHost: {
      show: true,
      label: "Attacker forwarded host",
  ...
  LEVEL_4: {
    ...
    subtitle:
      "This level stops trusting <code>X-Forwarded-Host</code>, but still caches personalized content publicly. The attacker request sets <code>demo_user</code>; the victim request clears that cookie before loading the same route.",
    banner: { show: false },
    forwardedHost: { show: false },
    demoUser: {
      show: true,
      label: "Attacker demo_user cookie",
      placeholder: "admin",
    },
    poisonBtn: "demo",
    signals: ["status", "key", "cacheControl", "vary"],
  ...
  LEVEL_5: {
    ...
    banner: {
      show: true,
      label: "Optional banner preview",
      placeholder: "<b>OWNED</b>",
    },
    forwardedHost: {
      show: true,
      label: "Optional forwarded host",
      placeholder: "attacker.example",
    },
    demoUser: {
      show: true,
      label: "Personalized demo_user",
      placeholder: "admin",
    },
    poisonBtn: "demo",
    signals: ["status", "key", "cacheControl", "vary"],
...
function updateDiagnostics(request) {
  ...
  if (document.getElementById("cacheControl"))
    document.getElementById("cacheControl").textContent = cacheControl;
  if (document.getElementById("varyHeader"))
    document.getElementById("varyHeader").textContent = vary;
...
function initLevel() {
  ...
  document.getElementById("cachePoisoningResponse").innerHTML =
    config.responseInitial;
  document.getElementById("victimRequestBtn").textContent =
    config.victimBtnText;
  ...
  if (config.banner.show) {
    ...
    document.getElementById("bannerInput").placeholder =
      config.banner.placeholder;
  }

  if (config.forwardedHost.show) {
    document.getElementById("forwardedHostInputRow").classList.remove("hidden");
    document.getElementById("forwardedHostLabel").textContent =
      config.forwardedHost.label;
    document.getElementById("forwardedHostInput").placeholder =
      config.forwardedHost.placeholder;
  }

  if (config.demoUser.show) {
    document.getElementById("demoUserInputRow").classList.remove("hidden");
    document.getElementById("demoUserLabel").textContent =
      config.demoUser.label;
    document.getElementById("demoUserInput").placeholder =
      config.demoUser.placeholder;
  }

  // Configure Poison Button Position
  const poisonBtnId =
    config.poisonBtn === "inline"
      ? "poisonCacheBtnInline"
      : config.poisonBtn === "demo"
      ? "poisonCacheBtnDemo"
      : "poisonCacheBtnBottom";
  const poisonBtnContainerId =
    config.poisonBtn === "inline"
      ? "poisonCacheBtnContainerInline"
      : config.poisonBtn === "demo"
      ? "poisonCacheBtnContainerDemo"
      : "";
  ...
  if (config.signals.includes("cacheControl"))
    document.getElementById("cacheControlCard").classList.remove("hidden");
  if (config.signals.includes("vary"))
    document.getElementById("varyHeaderCard").classList.remove("hidden");
}
